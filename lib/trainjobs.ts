...
// phone-side: build the dataset from the city pool and open a job
export async function startTrainingJob(
  user: { id: string }, team: string | null,
  onProgress: (msg: string) => void,
): Promise<{ job: TrainJob } | { error: string }> {
  onProgress('אוסף את מאגר העיר…');
  const built = await buildCityPoolZip((d, t) => onProgress(`אורז ${d}/${t} תמונות…`));
  if (!built || !built.count) {
    return { error: 'אין עדיין תמונות מתויגות (עם תיבות) בפול — צריך תפיסות דרך מודל YOLO, או תיוג בדסקטופ.' };
  }
  ...
  const { data, error } = await sb.from('sc_training_jobs').insert({
    ...
    requested_by: user.id, team_name: team,
  }).select().single();
